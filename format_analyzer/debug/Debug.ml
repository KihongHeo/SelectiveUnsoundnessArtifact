...
[TABLE] [FID] [NODE]   Print a memory in the table.
memFI                  Print a memory of flow insensitive analysis.
fs_locs                Print a set of locations for flow sensitive.
  ...
  if node = "exit" then (fname, IntraNode.Exit) else
    let i =
      try int_of_string node with
        Failure "int_of_string" -> raise Invalid_dbg in
...
let elements_of_mem_sum m =
  (Mem.Mem.elements (Mem.base m),
   Mem.Mem.elements (Mem.spec m))
...
let query_loc m =
  let (base_elems, spec_elems) = elements_of_mem_sum m in
  let base_size = List.length base_elems in
  let spec_size = List.length spec_elems in
  let total_size = base_size + spec_size in
  PPVocab.pp_endline (print_keys "base" base_elems) 0;
  PPVocab.pp_endline (print_keys "spec" spec_elems) base_size;
  ...
  if str = "" then None else
    try
      ...
      if 0 <= i && i < total_size then
        if i < base_size then Some (snd (List.nth base_elems i)) else
          Some (snd (List.nth spec_elems (i - base_size)))
      else raise Invalid_loc
    with Failure "int_of_string" -> raise Invalid_loc
...
let rec debug (g, memFI, fs_locs, access_map, dug, io, oo, d_io, d_oo) =
  let dbg_cmd =
  ...
  let continue =
    ...
    | Mem (t, node) ->
      let tbl =
        ...
        | DInput -> d_io
        | DOutput -> d_oo in
      let m = Table.find node tbl in
      query_mem m; true
    | MemFI -> query_mem memFI; true
    | FSLocs -> PPVocab.pp_endline PPMem.print_pow_loc fs_locs; true
    | Cmd node ->
      (try
         let icfg = G.icfg g in
         let cmd = default (Syn.Cskip Pos_as_OT.unknown_pos) (InterCfg.get_cmd icfg node) in
  ...
  if continue then
    debug (g, memFI, fs_locs, access_map, dug, io, oo, d_io, d_oo)
  else ()
