 ...
 struct 
  module E2 = 
   struct 
    ...
    let coq_ILat =
      { DLat.le_dec = le_dec; DLat.eq_dec = eq_dec; DLat.bot = bot;
        DLat.join = join; DLat.meet = meet; DLat.widen = widen; DLat.narrow =
        narrow }
   end
  ...
  let plus_offset x i =
    let (t0, st) = x in let (o, s) = t0 in (((Itv.plus o i), s), st)
  
  (** val minus_offset : t -> Itv.t -> t **)
  
  let minus_offset x i =
    let (t0, st) = x in let (o, s) = t0 in (((Itv.minus o i), s), st)
 end
 ...
 struct 
  module A = 
   struct 
    ...
    (** val compare :
        ...
        string_t) sum) sum -> (string_t * t', (unit, string_t) sum) sum
  ...
  module M = 
   struct 
    ...
    module Raw = 
     struct 
      type key = (string_t * t', (unit, string_t) sum) sum
      ...
      (** val mem :
          (string_t * t', (unit, string_t) sum) sum -> 'a1 tree -> bool **)
      ...
      (** val find :
          (string_t * t', (unit, string_t) sum) sum -> 'a1 tree -> 'a1 option **)
      ...
      (** val remove :
          (string_t * t', (unit, string_t) sum) sum -> 'a1 tree -> 'a1 tree **)
      ...
      (** val split :
          (string_t * t', (unit, string_t) sum) sum -> 'a1 tree -> 'a1 triple **)
      ...
      (** val equal_more :
          ('a1 -> 'a1 -> bool) -> (string_t * t', (unit, string_t) sum) sum
       ...
       struct 
        module MX = 
         struct 
          ...
          (** val eq_dec :
              (string_t * t', (unit, string_t) sum) sum -> (string_t * t',
              (unit, string_t) sum) sum -> bool **)
        ...
        module L = 
         struct 
           ...
           struct 
            module MO = 
             struct 
              ...
              (** val eq_dec :
                  (string_t * t', (unit, string_t) sum) sum ->
                  (string_t * t', (unit, string_t) sum) sum -> bool **)
          ...
          type 'elt t =
            ((string_t * t', (unit, string_t) sum) sum * 'elt) list
          ...
          type 'elt coq_R_mem =
          | R_mem_0 of 'elt t
          | R_mem_1 of 'elt t * (string_t * t', (unit, string_t) sum) sum
             * 'elt * ((string_t * t', (unit, string_t) sum) sum * 'elt) list
          | R_mem_2 of 'elt t * (string_t * t', (unit, string_t) sum) sum
             * 'elt * ((string_t * t', (unit, string_t) sum) sum * 'elt) list
          | R_mem_3 of 'elt t * (string_t * t', (unit, string_t) sum) sum
             * 'elt * ((string_t * t', (unit, string_t) sum) sum * 'elt) list
             * bool * 'elt coq_R_mem
          
          (** val coq_R_mem_rect :
              key -> ('a1 t -> __ -> 'a2) -> ('a1 t -> (string_t * t', (unit,
              string_t) sum) sum -> 'a1 -> ((string_t * t', (unit, string_t)
              sum) sum * 'a1) list -> __ -> __ -> __ -> 'a2) -> ('a1 t ->
              (string_t * t', (unit, string_t) sum) sum -> 'a1 ->
              ((string_t * t', (unit, string_t) sum) sum * 'a1) list -> __ ->
              __ -> __ -> 'a2) -> ('a1 t -> (string_t * t', (unit, string_t)
              sum) sum -> 'a1 -> ((string_t * t', (unit, string_t) sum)
              sum * 'a1) list -> __ -> __ -> __ -> bool -> 'a1 coq_R_mem ->
          ...
          (** val mem_rect :
              ...
              __ -> __ -> 'a2) -> ('a1 t -> (string_t * t', (unit, string_t)
              sum) sum -> 'a1 -> ((string_t * t', (unit, string_t) sum)
              sum * 'a1) list -> __ -> __ -> __ -> 'a2 -> 'a2) -> 'a1 t ->
              'a2 **)
          ...
          type 'elt coq_R_find =
          | R_find_0 of 'elt t
          | R_find_1 of 'elt t * (string_t * t', (unit, string_t) sum) sum
             * 'elt * ((string_t * t', (unit, string_t) sum) sum * 'elt) list
          | R_find_2 of 'elt t * (string_t * t', (unit, string_t) sum) sum
             * 'elt * ((string_t * t', (unit, string_t) sum) sum * 'elt) list
          | R_find_3 of 'elt t * (string_t * t', (unit, string_t) sum) sum
             * 'elt * ((string_t * t', (unit, string_t) sum) sum * 'elt) list
          ...
          (** val coq_R_find_rect :
              ...
              __ -> __ -> 'a2) -> ('a1 t -> (string_t * t', (unit, string_t)
              sum) sum -> 'a1 -> ((string_t * t', (unit, string_t) sum)
              sum * 'a1) list -> __ -> __ -> __ -> 'a1 option -> 'a1
          ...
          type 'elt coq_R_add =
          | R_add_0 of 'elt t
          | R_add_1 of 'elt t * (string_t * t', (unit, string_t) sum) sum
             * 'elt * ((string_t * t', (unit, string_t) sum) sum * 'elt) list
          | R_add_2 of 'elt t * (string_t * t', (unit, string_t) sum) sum
             * 'elt * ((string_t * t', (unit, string_t) sum) sum * 'elt) list
          | R_add_3 of 'elt t * (string_t * t', (unit, string_t) sum) sum
             * 'elt * ((string_t * t', (unit, string_t) sum) sum * 'elt) list
             * 'elt t * 'elt coq_R_add
          
          (** val coq_R_add_rect :
              key -> 'a1 -> ('a1 t -> __ -> 'a2) -> ('a1 t -> (string_t * t',
              (unit, string_t) sum) sum -> 'a1 -> ((string_t * t', (unit,
              string_t) sum) sum * 'a1) list -> __ -> __ -> __ -> 'a2) ->
              ('a1 t -> (string_t * t', (unit, string_t) sum) sum -> 'a1 ->
              ((string_t * t', (unit, string_t) sum) sum * 'a1) list -> __ ->
              __ -> __ -> 'a2) -> ('a1 t -> (string_t * t', (unit, string_t)
              sum) sum -> 'a1 -> ((string_t * t', (unit, string_t) sum)
              sum * 'a1) list -> __ -> __ -> __ -> 'a1 t -> 'a1 coq_R_add ->
          ...
          | R_remove_0 of 'elt t
          | R_remove_1 of 'elt t * (string_t * t', (unit, string_t) sum) sum
             * 'elt * ((string_t * t', (unit, string_t) sum) sum * 'elt) list
          | R_remove_2 of 'elt t * (string_t * t', (unit, string_t) sum) sum
             * 'elt * ((string_t * t', (unit, string_t) sum) sum * 'elt) list
          | R_remove_3 of 'elt t * (string_t * t', (unit, string_t) sum) sum
             * 'elt * ((string_t * t', (unit, string_t) sum) sum * 'elt) list
          ...
          (** val coq_R_remove_rect :
              ...
              __ -> __ -> 'a2) -> ('a1 t -> (string_t * t', (unit, string_t)
              sum) sum -> 'a1 -> ((string_t * t', (unit, string_t) sum)
              sum * 'a1) list -> __ -> __ -> __ -> 'a1 t -> 'a1 coq_R_remove
          ...
          | R_fold_1 of (key -> 'elt -> 'a -> 'a) * 'elt t * 'a
             * (string_t * t', (unit, string_t) sum) sum * 'elt
             * ((string_t * t', (unit, string_t) sum) sum * 'elt) list * 
             'a * ('elt, 'a) coq_R_fold
          ...
          (** val coq_R_equal_rect :
              ...
              ('a1 t -> 'a1 t -> (string_t * t', (unit, string_t) sum) sum ->
              'a1 -> ((string_t * t', (unit, string_t) sum) sum * 'a1) list
              -> __ -> (string_t * t', (unit, string_t) sum) sum -> 'a1 ->
              ((string_t * t', (unit, string_t) sum) sum * 'a1) list -> __ ->
          ...
          (** val equal_rect :
              ...
              -> __ -> (string_t * t', (unit, string_t) sum) sum -> 'a1 ->
              ((string_t * t', (unit, string_t) sum) sum * 'a1) list -> __ ->
              __ -> __ -> 'a2 -> 'a2) -> ('a1 t -> 'a1 t -> (string_t * t',
              (unit, string_t) sum) sum -> 'a1 -> ((string_t * t', (unit,
              string_t) sum) sum * 'a1) list -> __ -> (string_t * t', (unit,
              string_t) sum) sum -> 'a1 -> ((string_t * t', (unit, string_t)
              sum) sum * 'a1) list -> __ -> (string_t * t', (unit, string_t)
              sum) sum OrderedType.coq_Compare -> __ -> __ -> 'a2) -> ('a1 t
        ...
        (** val coq_R_mem_rect :
            (string_t * t', (unit, string_t) sum) sum -> ('a1 tree -> __ ->
        ...
        (** val coq_R_split_rect :
            ...
            -> 'a1 tree -> Int.Z_as_Int.t -> __ -> __ -> __ -> 'a1 triple ->
    ...
    (** val map_induction_max :
        ...
        (unit, string_t) sum) sum -> 'a1 -> __ -> __ -> 'a2) -> 'a1 t -> 'a2 **)
    ...
    module Raw2 = 
     struct 
      ...
      let rec coq_R_for_all_rect print3 cond f f0 m b = function
      | R_for_all_0 m0 -> f m0 __
      | R_for_all_1 (m0, l, x, d, r0, _x, res0, r1, res, r2) ->
        f0 m0 l x d r0 _x __ res0 r1
          (coq_R_for_all_rect print3 cond f f0 l res0 r1) res r2
          (coq_R_for_all_rect print3 cond f f0 r0 res r2)
      ...
      let rec coq_R_for_all_rec print3 cond f f0 m b = function
      | R_for_all_0 m0 -> f m0 __
      | R_for_all_1 (m0, l, x, d, r0, _x, res0, r1, res, r2) ->
        f0 m0 l x d r0 _x __ res0 r1
          (coq_R_for_all_rec print3 cond f f0 l res0 r1) res r2
          (coq_R_for_all_rec print3 cond f f0 r0 res r2)
  ...
  (** val find :
      (string_t * t', (unit, string_t) sum) sum -> t -> ArrInfo.t **)
  ...
  (** val add :
      (string_t * t', (unit, string_t) sum) sum -> ArrInfo.t -> t -> t **)
  ...
  (** val remove : (string_t * t', (unit, string_t) sum) sum -> t -> t **)
  ...
  (** val mapi :
      ((string_t * t', (unit, string_t) sum) sum -> ArrInfo.t -> ArrInfo.t)
      -> t -> t **)
  ...
  (** val foldi :
      ((string_t * t', (unit, string_t) sum) sum -> ArrInfo.t -> 'a1 -> 'a1)
      -> t -> 'a1 -> 'a1 **)
  ...
  (** val filteri :
      ((string_t * t', (unit, string_t) sum) sum -> ArrInfo.t -> bool) -> t
      -> t **)
  ...
  (** val elements :
      t -> ((string_t * t', (unit, string_t) sum) sum * ArrInfo.t) list **)
  ...
  (** val le_than :
      t -> (string_t * t', (unit, string_t) sum) sum -> ArrInfo.t -> bool **)
  
  let le_than y k v =
    if ArrInfo.le_dec v (find k y) then true else false
  
  (** val for_all :
      ((string_t * t', (unit, string_t) sum) sum -> ArrInfo.t -> unit) ->
      ((string_t * t', (unit, string_t) sum) sum -> ArrInfo.t -> bool) -> t
      -> bool **)
  ...
  (** val unstables :
      t -> t -> (ArrInfo.t -> ArrInfo.t -> bool) -> (string_t * t', (unit,
      string_t) sum) sum list -> (((string_t * t', (unit, string_t) sum)
      sum * ArrInfo.t) * ArrInfo.t) list **)
  ...
  (** val coq_IMap :
      ((string_t * t', (unit, string_t) sum) sum, ArrInfo.t, t) coq_TCMap **)
  ...
  let cast_array new_st ab =
    ...
    let cast_offset = fun x ->
      ...
      let new_s = resize orig_st s in ((new_o, new_s), new_st)
    in
    map cast_offset ab
  ...
  let pow_loc_of_array ab =
    let add_array_to_pow_loc = fun k x acc ->
      PowLoc.coq_ISet.set_add (loc_of_allocsite k) acc
    in
    foldi add_array_to_pow_loc ab PowLoc.coq_ISet.set_empty
  
  (** val pow_loc_of_struct_w_field : t -> Field.t -> PowLoc.t **)
  
  let pow_loc_of_struct_w_field ab f =
    let add_to_pow_loc = fun a x ->
      PowLoc.coq_ISet.set_add (append_field (loc_of_allocsite a) f)
    in
