...
module ProcMap = FMapAVL'.Make(Proc)
...
module Loc = ProdKey2(VarAllocsite)(Fields)

module PowLoc = Pow(Loc)

module LocMap = FMapAVL'.Make(Loc)

module ExtProcPos = ProdKey2(Proc)(DPos.DPos)

module PowExtProcPos = Pow(ExtProcPos)
...
module Dump = 
 struct 
             ...
             | p :: l ->
               ...
               (match m' with
                ...
                | p0 :: l0 ->
                  ...
                  let f12 = let _x = Proc.compare t0 t1 in f11 _x __ in
                  let f13 = f10 t1 e0 l0 __ in
                  let f14 = fun _ _ ->
                    let hrec = equal_rect cmp f2 f1 f0 f l l0 in
                    f13 __ __ hrec
                  in
                  (match Proc.compare t0 t1 with
  ...
  let weak_add k v m =
    ...
    if PowLoc.le_dec v orig_v then m else M.add k (PowLoc.join orig_v v) m
  
  (** val fast_weak_add : string_t -> PowLoc.t -> t -> t **)
  
  let fast_weak_add k v m =
    let orig_v = find k m in M.add k (PowLoc.join orig_v v) m
  ...
  let le_than y k v =
    if PowLoc.le_dec v (find k y) then true else false
  ...
  let meet_big_small x y =
    let iter1 = fun k v -> add k (PowLoc.meet (find k x) v) in
    foldi iter1 y y
  ...
  let strong_le x y =
    M.strong_le (fun x0 y0 -> if PowLoc.le_dec x0 y0 then true else false) x
      y
  ...
  let join' opt_v1 opt_v2 =
    ...
    | Some v1 ->
      (match opt_v2 with
       | Some v2 ->
         let joined_v = PowLoc.join v1 v2 in
         if PowLoc.eq_dec joined_v PowLoc.bot then None else Some joined_v
       | None -> if PowLoc.eq_dec v1 PowLoc.bot then None else Some v1)
    | None ->
      (match opt_v2 with
       | Some v -> if PowLoc.eq_dec v PowLoc.bot then None else Some v
       | None -> None)
  ...
  let meet' opt_v1 opt_v2 =
    ...
    | Some v1 ->
      (match opt_v2 with
       | Some v2 ->
         let meeted_v = PowLoc.meet v1 v2 in
         if PowLoc.eq_dec meeted_v PowLoc.bot then None else Some meeted_v
       | None -> None)
    | None -> None
  ...
  let widen' opt_v1 opt_v2 =
    ...
    | Some v1 ->
      (match opt_v2 with
       | Some v2 ->
         let widened_v = PowLoc.widen v1 v2 in
         if PowLoc.eq_dec widened_v PowLoc.bot then None else Some widened_v
       | None -> if PowLoc.eq_dec v1 PowLoc.bot then None else Some v1)
    | None ->
      (match opt_v2 with
       | Some v -> if PowLoc.eq_dec v PowLoc.bot then None else Some v
  ...
  let narrow' opt_v1 opt_v2 =
    ...
    | Some v1 ->
      (match opt_v2 with
       | Some v2 ->
         let narrowed_v = PowLoc.narrow v1 v2 in
         if PowLoc.eq_dec narrowed_v PowLoc.bot
         then None
  ...
  let remove_node node dump =
    let is_not_same_node = fun l ->
      let (y, y0) = l in
      (match y with
       | Coq_inl y1 -> true
       | Coq_inr y1 ->
         (match y1 with
          | Coq_inl node' ->
            if InterNode.eq_dec node node' then false else true
          | Coq_inr y2 -> true))
    in
    map (PowLoc.filter is_not_same_node) dump
  
  (** val powloc_remove_function : pid_t -> PowLoc.t -> PowLoc.t **)
  
  let powloc_remove_function f ls =
    let is_not_f = fun l ->
      let (y, y0) = l in
      (match y with
       | Coq_inl y1 ->
       ...
       | Coq_inr y1 ->
         (match y1 with
          ...
          | Coq_inr y2 -> true))
    in
    PowLoc.filter is_not_f ls
  
  (** val remove_function : pid_t -> t -> t **)
  
  let remove_function f dump =
    let filter_f = fun g ls acc ->
      if InterCfg.Pid.eq_dec f g
      then acc
      else add g (powloc_remove_function f ls) acc
    in
    foldi filter_f dump empty
 end
