...
let t_cmd = function
  ...
  | Cmd.Cskip -> Syn.Cskip Pos_as_OT.unknown_pos
...
let t_intra_cfg cfg =
  ...
  let cmds =
    ...
    t_batmap t_intra_node t_cmd add init cfg.FIntraCfg.cmd_map in
  let add_skip node cmds =
    IntraCfg.NodeMap.add node (Syn.Cskip Pos_as_OT.unknown_pos) cmds in
...
let t_cfgs cfgs = t_batmap id t_intra_cfg PidMap.add PidMap.empty cfgs
...
let t_inter_cfg icfg =
  ...
  ; InterCfg.nodes =
      (let add node = InterCfg.NodeSet.add (t_inter_node node) in
       ...
       list_fold add (FInterCfg.nodesof icfg) init)
  }
...
let t_inter_edges g =
  let icfg = UserInput.Input.G.icfg g in
  let cg = UserInput.Input.G.callgraph g in
  let (succ, pred) = t_inter_edges' icfg cg in
  let icfg = { icfg with InterCfg.succ = succ ; InterCfg.pred = pred } in
  { g with UserInput.Input.G.icfg = icfg }
