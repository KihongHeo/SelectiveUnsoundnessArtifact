...
module Oper = Graph.Oper.P (DFG)
module CC = Components.Make (DFG)

module VL = struct
  type t = (DFG.V.t * PowLoc.t)
  let compare (v, l) (v', l') =
    if DFG.V.compare v v' = 0 &&
       PowLoc.eq_dec l l' then 0 else
    if DFG.V.compare v v' = 0 &&
       PowLoc.le_dec l l' then -1 else
    if DFG.V.compare v v' = 0 &&
       not(PowLoc.le_dec l l') then 1 else
    DFG.V.compare v v'
end
module VMap = Map.Make (DFG.V)
module VSet = Set.Make ( DFG.V )
...
let checkLabel prev cur setLoc (g, cache) inputof =
  let powloc = get_abslocs cur prev (g,cache) in
  (* let mem = Table.find prev inputof in                                         *)
  (* let powloc = PowLoc.filter (fun loc ->                                       *)
  (*     let idM = (Obj.magic DomMem.coq_IdMem) in                                *)
  (*     let mId = (Obj.magic DomMem.coq_MId) in                                  *)
  (*     let v =                                                                  *)
  (*       pow_proc_pos_of_val (mem_lookup idM mId (PowLoc.singleton loc) mem) in *)
  (*     not (PowExtProcPos.is_empty v)                                           *)
  (*   ) powloc in                                                                *)
  (* let _ = prerr_endline (PPMem.tostring_pow_loc powloc) in                     *)
  PowLoc.is_empty (PowLoc.meet setLoc powloc)

let isVisited vertex setLoc mapV  =
  try let l = VMap.find vertex mapV in
    if PowLoc.subset setLoc l then true else false
  with Not_found -> false

let get_ordinary_uses pre node = Acc.useof (Pre.get_access pre node)

let get_not_bot_powloc powloc mem =
  PowLoc.filter (fun loc ->
    let idM = (Obj.magic DomMem.coq_IdMem) in
    let mId = (Obj.magic DomMem.coq_MId) in
    let v = pow_proc_pos_of_val (mem_lookup idM mId (PowLoc.singleton loc) mem) in
    not (PowExtProcPos.is_empty v)
    ) powloc 
      

let cut_edges src sink ext (g, cache) oo mem =
  let ext = PowExtProcPos.singleton ext in
  let _ = PPMem.print_pow_proc_pos ext in
  let edge_iter e g =
    let locs = DFG.E.label e in
    let mem = Table.find (DFG.E.src e) oo in
    let idM = (Obj.magic DomMem.coq_IdMem) in
    let mId = (Obj.magic DomMem.coq_MId) in
    let v = pow_proc_pos_of_val (mem_lookup idM mId locs mem) in
    if (PowExtProcPos.is_empty (PowExtProcPos.meet ext v)) then
      DFG.remove_edge_e g e
    else
      g
  in
  let g = DFG.fold_edges_e edge_iter g g in
  g
     
   
let rec get_nodes src (g, cache) icfg prevNode preds setLoc mapV pre inputof =
  let process_pred mapV pred =
    if checkLabel prevNode pred setLoc (g, cache) inputof then
      mapV else
    if (isVisited pred setLoc mapV) then
      mapV else
    if DFG.V.compare src pred = 0 then
      VMap.add pred setLoc mapV else
      let _ = prerr_endline "test" in
    let mem = Table.find pred inputof in
    let cmd = (default (Syn.Cskip Pos.Pos_as_OT.unknown_pos)
                 (InterCfg.get_cmd icfg pred)) in
       
    let mapV = VMap.add pred setLoc mapV in

    match cmd with
    | Syn.Cskip _ ->
      (get_nodes src (g, cache) icfg pred (DFG.pred g pred) setLoc mapV pre inputof)

    | Syn.Ccall (ret_opt, _, _, _) ->
      (match ret_opt with
      | Some lv ->
        let preds = DFG.pred g pred in
        let addLoc = get_not_bot_powloc (get_ordinary_uses pre pred) mem in
        let setLoc = PowLoc.union setLoc addLoc in
        (get_nodes src (g, cache) icfg pred preds setLoc mapV pre inputof)
      | None ->
        let addLoc = get_not_bot_powloc (get_ordinary_uses pre pred) mem in
        let setLoc = PowLoc.union setLoc addLoc in
        (get_nodes src (g, cache) icfg pred (DFG.pred g pred) setLoc mapV pre inputof)
...
(*      let preds = DFG.pred g pred in *)
      (*
      let preds_iter acc pred' =
        PowLoc.union setLoc (get_abslocs pred' pred (g,cache))
      in
      let setLoc = List.fold_left preds_iter setLoc preds in *)
      (*
      let setLoc = PowLoc.union setLoc (get_ordinary_uses pre pred) in 
      (get_nodes src (g, cache) icfg pred preds setLoc mapV pre)  *)
      (get_nodes src (g, cache) icfg pred (DFG.pred g pred) setLoc mapV pre inputof)
    in
    List.fold_left process_pred mapV preds 
...
let cut_suprious_info src sink (g, cache) icfg pre m inputof =
  let _ = prerr_endline ("G size is " ^ (string_of_int (DFG.nb_vertex g))) in
  let sink_preds = DFG.pred g sink in
  let setLoc = get_powloc_expr icfg sink m in
  let _ = prerr_endline (PPMem.tostring_pow_loc setLoc) in
  let mapV = VMap.add src setLoc (VMap.add sink setLoc VMap.empty) in
  let mapV = get_nodes src (g, cache) icfg sink sink_preds setLoc mapV pre inputof
  in
  let remove_vertex v g =
    try let _ = VMap.find v mapV in
      g
    with Not_found ->
      DFG.remove_vertex g v
  in
  let g = DFG.fold_vertex remove_vertex g g in
  let _ = prerr_endline ("Cut G size is " ^ (string_of_int (DFG.nb_vertex g))) in
  g
  
(** return a subgraph of paths from src to sink. *)
let between_src_sink src sink graph icfg pre m src_nodes inputof =
  let g = cut_suprious_info src sink graph icfg pre m inputof in
  (*
  let iter_vertex v g =
    (* For rptp *)
    (*
    if InterNode.get_pid v = "do_application" &&
    ...
    *)
    g
  in*)
  (* let g = DFG.fold_vertex iter_vertex g g in *)
  (* let g = cut_suprious_info src sink graph icfg in *)
  (* 
  let checker = Check.create g in
  let iter_vertex v acc =
    if (Check.check_path checker src v) &&
       (Check.check_path checker v sink) then
      v :: acc
    else acc
  in
  let nodes = DFG.fold_vertex iter_vertex g [] in
  let nodes_iter g' node =
    let preds = List.filter (fun p -> List.mem p nodes) (DFG.pred g node) in
    let succs = List.filter (fun s -> List.mem s nodes) (DFG.succ g node) in
    let g' = List.fold_left (fun g' p -> DFG.add_edge g' p node) g' preds in
    let g' = List.fold_left (fun g' s -> DFG.add_edge g' node s) g' succs in
    g' in
  List.fold_left nodes_iter DFG.empty nodes
  *)
  g
...
let locals_of_function pre f = Pre.get_access_local pre f
let locals_of_program pre = Pre.get_access_local_program pre

let uses_of_function pre f ls =
  PowLoc.intersect ls (Acc.useof (Pre.get_access_reach pre f))
let defs_of_function pre f ls =
  PowLoc.intersect ls (Acc.defof (Pre.get_access_reach pre f))
let access_of_function pre f ls =
  let defs = Acc.defof (Pre.get_access_reach pre f) in
  let uses = Acc.useof (Pre.get_access_reach pre f) in
  PowLoc.intersect ls (PowLoc.union defs uses)

let find_callees node g =
  let cg = G.callgraph g in
  let node_calls = Global.Callgraph.node_calls cg in
  match InterCfg.NodeMap.find node node_calls with
  | Some v -> v
  | None -> InterCfg.PidSet.empty

let get_local_locations pre f = Acc.accessof (Pre.get_access_proc pre f)

(** locations considered as being used in the given node *)
let lhsof (g, pre) node ls =
  ...
  let defs_at_entry =
    if InterNode.is_entry_node node then
      access_of_function pre (InterNode.get_pid node) ls
    else PowLoc.empty in
  (* return nodes define defs(callees) *)
  let union_defs_f f = PowLoc.union (defs_of_function pre f ls) in
  let defs_at_return =
...
let rec rhsof (g, pre) node ls =
  let f = InterNode.get_pid node in
  let icfg = G.icfg g in
  let ordinary_uses = get_ordinary_uses pre node in
  (* exit uses defs(f) *)
  let uses_at_exit =
    if InterNode.is_exit_node node then defs_of_function pre f ls else
      PowLoc.empty in
  (* return node *)
  let uses_at_return =
    if InterCfg.is_return_node icfg node then
      ...
      | Some call_node ->
        ...
        let defs_callee = lhsof (g, pre) call_node ls in
        (* return node uses not-localized variables of call node *)
        let callees = find_callees call_node g in
        let defs_of_callees =
          let add_defs callee =
            PowLoc.union (defs_of_function pre callee ls) in
          InterCfg.PidSet.fold add_defs callees PowLoc.empty in
        let add_not_defined callee =
          let defs = defs_of_function pre callee ls in
          PowLoc.union (PowLoc.diff defs_of_callees defs) in
        let non_localized =
          InterCfg.PidSet.fold add_not_defined callees PowLoc.empty in
        PowLoc.union defs_callee non_localized
      | None -> PowLoc.empty
    else PowLoc.empty in
  (* call nodes uses access(callees) *)
  let union_access_f f = PowLoc.union (access_of_function pre f ls) in
  let uses_at_call =
...
let prepare_defs_uses (g, pre, ls) (f, cfg) =
  let collect func =
    let add_func_result node =
      let inter_node = (f, node) in
      IntraCfg.NodeMap.add node (func (g, pre) inter_node ls) in
...
let cfg2dug (g, pre, ls) (f, cfg) (dug, dom_tree) =
  ...
  let uses_of node =
    let v = get_some (IntraCfg.NodeMap.find node node2uses) in
(*
    if f = "main" && node = IntraNode.Node 3631 then
      ( PPMem.print_pow_loc v; Format.print_newline () );
*)
    v
  in
  let phi_vars_of node =
    let v = default PowLoc.empty (InterCfg.NodeMap.find (f, node) phi_points) in
    if f = "main" && node = IntraNode.Node 3631 then
      ( PPMem.print_pow_loc v; Format.print_newline () );
    v
  in
  let draw_from_lastdef loc2lastdef here loc dug =
    try
      ...
      let dst = (f, here) in
      if PowLoc.mem loc ls then add_absloc (G.icfg g) src loc dst dug else dug
    with _ -> dug in
...
let draw_intraedges (g, pre, ls) (dug, dom_tree) =
  ...
  let draw_intraedge f cfg (k, (dug, dom_tree)) =
    ...
    (k + 1, cfg2dug (g, pre, ls) (f, cfg) (dug, dom_tree)) in
  let (_, (dug, dom_tree)) =
    InterCfg.PidMap.fold draw_intraedge cfgs (1, (dug, dom_tree)) in
  (dug, dom_tree)

let draw_interedges (g, pre, ls) dug =
  let icfg = G.icfg g in
  ...
  let iter_call call (k, dug) =
    ...
    let iter_callee callee dug =
      ...
      let locs_on_call = access_of_function pre callee ls in
      let locs_on_return = defs_of_function pre callee ls in
      dug
...
let icfg2dug (g, pre, ls) =
  let pre = Pre.restrict_access pre ls in
  let (dug, dom_tree) =
    draw_intraedges (g, pre, ls) (empty, InterCfg.PidMap.empty) in
  let dug = draw_interedges (g, pre, ls) dug in
