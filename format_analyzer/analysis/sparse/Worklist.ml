...
let make dug : NGraph.t * (int, InterNodeG.t) BatMap.t =
  let i = ref 0 in
  ...
  fold_edges add_edge dug (NGraph.empty, BatMap.empty, BatMap.empty)
  |> fun (ng, i2n, _) -> (ng, i2n)
...
let rec perform (g, dug) =
  ...
  let trans_map trans_k trans_v m =
    ...
    BatMap.foldi add_1 m NodeMap.empty in
  let trans_set trans_v s =
    let add_1 v = NodeSet.add (trans_v v) in
    BatSet.fold add_1 s NodeSet.empty in
        ...
        match find_scc call sccs with
          ...
          | Some scc ->
            if List.length scc < 2 then NodeSet.remove call lhs else
              (* XXX: we assume that calls' successors are not calls *)
...
let init_nodes wo nodes =
  let init_v v = queue false NodeMap.empty v (NodeMap.find v wo) in
  list_fold init_v nodes WorkSet.empty

let init wo dug =
  let init_v v = queue false NodeMap.empty v (NodeMap.find v wo) in
  list_fold init_v (nodesof dug) WorkSet.empty
