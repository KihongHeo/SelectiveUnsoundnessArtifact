...
module Struct = PowDom.Make (struct 
                                type t = string 
                                let to_string x = x 
                                let compare = compare 
                             end)
...
struct
  ...
  let bot = { offset = Itv.bot; size = Itv.bot; stride = Itv.bot; null_pos = Itv.bot; structure = Struct.bot }
  let top = { offset = Itv.top; size = Itv.top; stride = Itv.top; null_pos = Itv.top; structure = Struct.bot }
  let input = { offset = Itv.zero; size = Itv.pos; stride = Itv.one; null_pos = Itv.nat; structure = Struct.bot }
  ...
  let join a1 a2 = 
    ...
    { offset    = Itv.join a1.offset a2.offset;
      ...
      structure = Struct.join a1.structure a2.structure; }
  ...
  let meet a1 a2 = 
    ...
    { offset    = Itv.meet a1.offset a2.offset;
      ...
      structure = Struct.meet a1.structure a2.structure; }
  ...
  let widen a1 a2 = 
    ...
    { offset    = Itv.widen a1.offset a2.offset;
      ...
      structure = Struct.widen a1.structure a2.structure; }
  ...
  let narrow a1 a2 = 
    ...
    { offset    = Itv.narrow a1.offset a2.offset;
      ...
      structure = Struct.narrow a1.structure a2.structure; }
  ...
  let eq a1 a2 = 
    if a1 == a2 then true
      ...
      && Struct.eq a1.structure a2.structure
  ...
  let le a1 a2 = 
    if a1 == a2 then true
      ...
      && Struct.le a1.structure a2.structure
 ...
 (* (o, s, orig_st, np, str) = *)
    match typ with
      Cil.TPtr ((Cil.TComp (comp, _) as t'), _) -> 
        ...
        { offset = resize arr.stride new_stride arr.offset;
          ...
          stride = new_stride;
          structure = Struct.add comp.Cil.cname arr.structure }
  ...
  let to_string arr = 
    ...
    ^", "^(Itv.to_string arr.null_pos)^", "^(Struct.to_string arr.structure)^")"
...
= fun a o sz st np ->
  add a (ArrInfo.make (o, sz, st, np, Struct.bot)) bot

let offsetof : t -> Itv.t
= fun a ->
  fold (fun arr -> Itv.join arr.ArrInfo.offset) a Itv.bot

let sizeof : t -> Itv.t
= fun a ->
  fold (fun arr -> Itv.join arr.ArrInfo.size) a Itv.bot

let nullof : t -> Itv.t
= fun a ->
  fold (fun arr -> Itv.join arr.ArrInfo.null_pos) a Itv.bot
...
let struct_of_array a = 
  foldi (fun k v -> 
      if Struct.bot <> v.ArrInfo.structure then PowLoc.add (Loc.of_allocsite k) 
      else id) a PowLoc.bot

let append_field : t -> Cil.fieldinfo -> PowLoc.t 
= fun s f ->
  foldi (fun a info ->
      if Struct.mem f.Cil.fcomp.Cil.cname info.ArrInfo.structure then
