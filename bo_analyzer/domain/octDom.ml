...
module OctLoc = 
struct 
  type t = Loc of Loc.t | Size of Allocsite.t
  let compare = compare
  let dummy = Loc (Loc.of_gvar "dummy")
  let of_loc l = Loc l
...
struct 
  ...
  = PowLoc.map OctLoc.of_loc
  let of_sizes : Allocsite.t BatSet.t -> t
  = BatSet.map OctLoc.of_size
  let to_string x = 
    if is_empty x then "{}"
    else to_string x
...
struct
  ...
  let get_pack packconf lv =
    ...
    with Not_found ->
      begin
      try
        let pack = BatSet.choose (BatSet.filter (Pack.mem lv) packconf) in
  ...
  let get_all_octlocs itvinputof = 
    ItvDom.Table.fold (fun mem pack ->
  ...
  let fullpack itvinputof = get_all_octlocs |> singleton

  let make itvinputof packs = 
    let all_octlocs = get_all_octlocs itvinputof in
    let relational = fold BatSet.union packs BatSet.empty in
    let non_relational = BatSet.diff all_octlocs relational in
    BatSet.fold (fun x -> BatSet.add (BatSet.singleton x)) non_relational packs
  ...
  let print_info packconf = 
    let total = BatSet.cardinal packconf in
    let (non_singleton, rel_var) = 
      BatSet.fold (fun x (pack, var) -> 
...
module Mem = 
struct 
  include MapDom.Make (Pack) (Octagon)

  let top packconf = 
    BatSet.fold (fun pack -> add pack (Octagon.top pack)) packconf bot
