...
let threshold = ref (BatSet.of_list [0;16;64])
let set_threshold l u = threshold := BatSet.of_list (gen_lst l u [])
...
let unknown_binary (x:t) (y:t) : t =
  ...
  else if !Options.opt_bugfinder >= 1 then x
  else top

let unknown_unary (x:t) : t = 
  if is_bot x then Bot 
  else if !Options.opt_bugfinder >= 1 then bot
  else top                    
...
let r_shift (x:t) (y:t) : t = unknown_binary x y
let b_xor (x:t) (y:t) : t = unknown_binary x y
let b_or (x:t) (y:t) : t = unknown_binary x y
let b_and (x:t) (y:t) : t = unknown_binary x y
let modulo (x:t) (y:t) : t = unknown_binary x y
let b_not (x:t) : t = unknown_unary x
