...
type t = {
  ...
  call_edges  : (Node.t, Proc.t BatSet.t) BatMap.t;
}
...
= fun call_node pid g -> 
  let callees = (try BatMap.find call_node g.call_edges with _ -> BatSet.empty)
    |> BatSet.add pid  
  in
  { g with call_edges = BatMap.add call_node callees g.call_edges }

let get_callees : Node.t -> t -> Proc.t BatSet.t
= fun call_node g ->
  try BatMap.find call_node g.call_edges with _ -> BatSet.empty 
...
let gen_cfgs file = 
  BatMap.add global_proc (IntraCfg.generate_global_proc file.Cil.globals (Cil.emptyFunction global_proc))
    (list_fold (fun g m ->
      match g with
      | Cil.GFun (f,loc) -> BatMap.add f.svar.vname (IntraCfg.fromFunDec f loc) m
      | _ -> m
...
=fun icfg ->
  ...
  |> (if !Options.opt_bb then (map_cfgs IntraCfg.opt_basic_block) else id)
  |> id
