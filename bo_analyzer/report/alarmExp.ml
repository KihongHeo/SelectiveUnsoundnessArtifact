...
=fun cmd ->
  ...
  | Cmd.Calloc (lv,e,_,loc) -> (c_lv lv loc) @ (c_exp e loc) 
  | Cmd.Csalloc (lv,_,loc) -> c_lv lv loc
  | Cmd.Cassume (e,loc) -> c_exp e loc
  | Cmd.Creturn (Some e, loc) -> c_exp e loc
  | Cmd.Cseq l -> List.fold_left (fun q c -> q@(collect c)) [] l
  | Cmd.Cinstr _ | Cmd.Cif (_, _, _, _) | Cmd.CLoop _ | Cmd.Cfalloc (_, _, _) | Cmd.Casm (_, _, _, _, _, _)
  | Cmd.Cskip | Cmd.Creturn (None, _) -> []
